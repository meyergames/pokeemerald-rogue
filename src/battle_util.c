...
{//                   Defender -->
 //  Attacker         Normal  Fighting Flying  Poison  Ground   Rock    Bug     Ghost   Steel  Mystery  Fire   Water   Grass  Electric Psychic   Ice   Dragon   Dark   Fairy   Stellar
    ...
    [TYPE_STELLAR]  = {______, ______, X(2.0), ______, ______, X(0.5), ______, ______, ______, ______, ______, X(2.0), X(0.5), ______, X(2.0), X(0.5), ______, X(0.5), ______, X(0.5)},
};
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
        ...
        case ABILITY_D2D_PACIFY:
            ...
            break;
        case ABILITY_D2D_OFF_BEAT:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                gBattlerAttacker = battler;
                BattleScriptPushCursorAndCallback(BattleScript_D2D_OffBeatActivates);
                effect++;
            }
            break;
...
bool32 IsBattlerProtected(u32 battler, u32 move)
{
    ...
    if ((GetBattlerAbility(gBattlerAttacker) == ABILITY_UNSEEN_FIST || GetBattlerAbility(gBattlerAttacker) == ABILITY_PIERCING_DRILL)
        ...
        return TRUE;
    else if (gProtectStructs[battler].detected)
        return TRUE;
...
static inline uq4_12_t GetFreezeModifier(u32 battlerDef, bool32 isCrit)
{
    if ((gBattleMons[battlerDef].status1 & STATUS1_FREEZE) && !isCrit)
        return UQ_4_12(0.25);
    return UQ_4_12(1.0);
}
                            ...
                            u32 holdEffectAtk, u32 holdEffectDef, u32 abilityAtk, u32 abilityDef)
{
    ...
    DAMAGE_APPLY_MODIFIER(GetFreezeModifier(battlerDef, isCrit));
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    if (gBattleMoves[move].effect == EFFECT_D2D_BLADE_SLASH && defType == TYPE_GRASS)
        mod = UQ_4_12(2.0);
    if (gBattleMoves[move].effect == EFFECT_D2D_CIRCUIT_BREAK && defType == TYPE_ELECTRIC)
        mod = UQ_4_12(2.0);
    if (gBattleMoves[move].effect == EFFECT_SKY_UPPERCUT && defType == TYPE_FLYING)
        mod = UQ_4_12(1.0);
    if (gBattleMoves[move].effect == EFFECT_D2D_MANDIBLE_JAW && defType == TYPE_BUG)
        mod = UQ_4_12(2.0);
