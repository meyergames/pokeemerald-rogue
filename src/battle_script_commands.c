...
static bool32 AccuracyCalcHelper(u16 move)
{
    if ((gStatuses3[gBattlerTarget] & STATUS3_ALWAYS_HITS && gDisableStructs[gBattlerTarget].battlerWithSureHit == gBattlerAttacker)
     ...
     || (move == MOVE_SUPERSONIC && IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_STELLAR))
    ...
    if (WEATHER_HAS_EFFECT)
    {
        ...
        else if (B_BLIZZARD_HAIL >= GEN_4 && (gBattleWeather & (B_WEATHER_HAIL | B_WEATHER_SNOW))
        && (move == MOVE_BLIZZARD || move == MOVE_D2D_SUBZERO))
        {
    ...
    else
    {
         ...
         && gBattleMons[battler].hp == 0)
        {
                 ...
                 && !IsDynamaxed(gBattlerAttacker))
            {
                ...
                gBattleMoveDamage = gBattleMons[destinyBondBattler].hp - 1; // D2D TODO: TEST THIS!
        ...
        case MOVEEND_PROTECT_LIKE_EFFECT:
            if (gProtectStructs[gBattlerAttacker].touchedProtectLike)
            {
                ...
                if (gProtectStructs[gBattlerTarget].detected)
                {
                    ...
                    // BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_D2D_DetectEffect;
                    effect = 1;
                }
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_TRY_AQUA_RING:
    {
        ...
        if (gStatuses4[gBattlerTarget] & STATUS4_AQUA_RING)
        {
            gBattlescriptCurrInstr = cmd->failInstr;
        }
        else
        {
            gStatuses4[gBattlerTarget] |= STATUS4_AQUA_RING;
            gBattlescriptCurrInstr = cmd->nextInstr;
        }
        return;
    }
